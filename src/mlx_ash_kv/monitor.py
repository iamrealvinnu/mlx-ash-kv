...
def run_monitor_demo():
    ...
    import mlx.core as mx
    from .cache import ASHCache
    cache = ASHCache(num_layers=4, num_heads=32, paging_enabled=True)
    tui = LiveDiagnosticTUI(cache)
    print("--- Launching ASH-KV Diagnostic Monitor ---")
    try:
        with Live(tui.get_layout(), refresh_per_second=4, screen=True):
            for i in range(100):
                new_k = mx.random.uniform(shape=(1, 32, 100, 128))
                new_v = mx.random.uniform(shape=(1, 32, 100, 128))
                cache.update_layer(0, new_k, new_v)
                cache.get_mask() # Trigger mutation
                time.sleep(0.1)
